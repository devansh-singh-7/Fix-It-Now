...
export async function isSuperAdmin(uid: string): Promise<boolean> {
  try {
    ...
    const user = await db
      .collection('users')
      .findOne({ firebaseUid: uid }, { projection: { email: 1, role: 1 } });
    ...
    console.log(
      `isSuperAdmin check: uid=${uid}, email=${user.email}, role=${user.role}, isSuperAdminEmail=${isSuperAdminEmail}, isSuperAdminUid=${isSuperAdminUid}, result=${result}`
    );
...
export async function createBuilding(
  ...
  try {
    ...
    // If ownerEmail is provided, look up that user and use them as the building admin
    let buildingAdminUid = adminUid;
    let shouldUpgradeToOwner = false;
    if (buildingData.ownerEmail) {
      const ownerUser = await getUserByEmail(buildingData.ownerEmail);
      if (!ownerUser) {
        throw new Error(`No registered user found with email: ${buildingData.ownerEmail}`);
      }
      buildingAdminUid = ownerUser.uid;
      // If user is not already admin or owner, upgrade them to owner role
      if (ownerUser.role !== 'admin' && ownerUser.role !== 'owner') {
        shouldUpgradeToOwner = true;
      }
    }
...
export async function getTicketsForUser(
  ...
  try {
    ...
    let tickets = await db.collection('tickets').find(filter).sort({ createdAt: -1 }).toArray();
    ...
    if (tickets.length === 0 && role === 'admin') {
      ...
      tickets = await db.collection('tickets').find({}).sort({ createdAt: -1 }).toArray();
    ...
    if (tickets.length === 0 && role === 'resident') {
      console.log(
        '[DB getTicketsForUser] No tickets with buildingId for resident, trying with createdBy only'
      );
      tickets = await db
        .collection('tickets')
        .find({ createdBy: uid })
  ...
  try {
    ...
    if (currentTicket) {
      ...
      await createNotification(
        ...
        'assignment',
        'New Ticket Assigned',
        `You have been assigned to: ${currentTicket.title}`,
        '🔧',
        `/tickets/${ticketId}`
      );

      // Notify the ticket creator that their ticket has been assigned
      if (currentTicket.createdBy) {
        await createNotification(
          currentTicket.createdBy,
          'ticket',
          'Ticket Assigned',
          `Your ticket "${currentTicket.title}" has been assigned to ${technicianName}`,
          '👷',
          `/tickets/${ticketId}`
        );
      }
 ...
 */
export async function getTechniciansForBuilding(buildingId: string): Promise<UserProfile[]> {
  try {
    const db = await getDatabase();
...
export async function updateBuilding(
  ...
  try {
    ...
    await db.collection('buildings').updateOne(filter, { $set: updateData });
...
export async function updateTicketStatusWithAuth(
  ...
  try {
    ...
    const statusMessages: Record<TicketStatus, { icon: string; message: string }> = {
      open: { icon: '📝', message: 'has been reopened' },
      assigned: { icon: '👷', message: 'has been assigned' },
      accepted: { icon: '✅', message: 'has been accepted by the technician' },
      in_progress: { icon: '🔧', message: 'is now in progress' },
      completed: { icon: '✨', message: 'has been marked as completed' },
    };

    const statusInfo = statusMessages[newStatus] || { icon: '📋', message: `status updated to ${newStatus}` };

    // Notify the ticket creator (resident) about status changes
    if (currentTicket.createdBy && currentTicket.createdBy !== userId) {
      await createNotification(
        currentTicket.createdBy,
        'ticket',
        'Ticket Status Update',
        `Your ticket "${ticketTitle}" ${statusInfo.message}`,
        statusInfo.icon,
        `/tickets/${ticketId}`
      );
    }

    // Notify the assigned technician about status changes (if not the one making the change)
    if (currentTicket.assignedTo && currentTicket.assignedTo !== userId) {
      await createNotification(
        currentTicket.assignedTo,
        'ticket',
        'Ticket Status Update',
        `Ticket "${ticketTitle}" ${statusInfo.message}`,
        statusInfo.icon,
        `/tickets/${ticketId}`
      );
    }

    // Notify admin when ticket is completed (if admin is not the one updating)
    if (newStatus === 'completed' && currentTicket.buildingId) {
      const building = await db.collection('buildings').findOne({ id: currentTicket.buildingId });
      if (building && building.adminId && building.adminId !== userId) {
        await createNotification(
          building.adminId,
          'ticket',
          'Ticket Completed',
          `Ticket "${ticketTitle}" has been completed by ${userName}`,
          '✅',
          `/tickets/${ticketId}`
        );
      }
