...
export async function GET(request: NextRequest, context: RouteContext) {
  try {
    ...
    // Fetch ticket with comments using the id field (not _id)
...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    // Add comment to ticket using the id field (not _id)
    const result = await db.collection('tickets').updateOne({ id: ticketId }, {
...
export async function DELETE(request: NextRequest, context: RouteContext) {
  try {
    ...
    // Get the ticket first to check permissions using the id field (not _id)
    const ticket = await db.collection('tickets').findOne({
      id: ticketId,
    });
    ...
    // Remove comment using the id field (not _id)
    await db.collection('tickets').updateOne({ id: ticketId }, {
